...
ALTER TABLE connected_account ADD COLUMN is_default BOOLEAN NOT NULL DEFAULT false;

-- Set existing accounts as default (one per user/provider)
UPDATE connected_account SET is_default = true;

-- Add unique constraint on user_id + provider + provider_account_id
CREATE UNIQUE INDEX connected_account_user_provider_account
